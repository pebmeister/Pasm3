/**
 * @file RegexEngine.hpp
 * @author Paul Baxter
 * @brief Lexical analyzer generator using Thompson NFA, Powerset DFA conversion, 
 *        Hopcroft minimization, and dynamic C++ tokenizer code generation.
 */
...
/**
 * @struct CharRange
 * @brief Represents an inclusive byte range \c [low, high] for character matching transitions.
 */
struct CharRange {
    unsigned char low;  /**< Lower bound byte value (inclusive). */
    unsigned char high; /**< Upper bound byte value (inclusive). */
};

/**
 * @struct NFAState
 * @brief Represents a state node within a Non-deterministic Finite Automaton (NFA).
 */
struct NFAState {
    int id;                /**< Unique identifier for the NFA state. */
    bool is_accept = false;/**< Flag indicating if this is an accepting/final state. */
    int token_id = 0;      /**< Associated token identifier returned upon matching. */
    bool anchor_bol = false; /**< Requires match at the beginning of a line / column 1 (^). */
    bool anchor_eol = false; /**< Requires match at the end of a line / EOF ($). */

    std::vector<NFAState*> epsilon_transitions; /**< Unconditional (\f$\epsilon\f$) transitions. */

    /**
     * @struct Transition
     * @brief Edge transition conditioned on matching specific character ranges.
     */
    struct Transition {
        std::vector<CharRange> ranges; /**< Accepted character byte ranges. */
        NFAState* target;              /**< Target NFA destination state. */
    };
    std::vector<Transition> transitions; /**< Outgoing character transitions. */
};

/**
 * @struct NFAFragment
 * @brief Sub-graph wrapper representing an NFA fragment with single entry and exit points.
 */
struct NFAFragment {
    NFAState* start = nullptr; /**< Entry state of the NFA fragment. */
    NFAState* end = nullptr;   /**< Exit state of the NFA fragment. */
};

/**
 * @class NFABuilder
 * @brief Parses regular expressions and constructs non-deterministic finite automata using Thompson's algorithm.
 */
class NFABuilder {
private:
    std::vector<std::unique_ptr<NFAState>> all_states_; /**< Memory owner for created NFA states. */
    int next_id_ = 0;                                   /**< Counter for generating unique state IDs. */
    bool case_insensitive_ = false;                    /**< Flag toggling case-folding behavior. */

    /**
     * @brief Parses and resolves escape sequences within regex character classes or expressions.
     * @param pattern Reference to the regex source pattern string.
     * @param pos In/Out cursor index within the pattern.
     * @return Decoded character byte.
     * @throws std::runtime_error If hex or octal escape sequences are malformed/out-of-bounds.
     */
    ...
    /**
     * @brief Applies case-folding expansion across a list of byte ranges.
     * @param input_ranges Vector of base character ranges.
     * @return Transformed vector of ranges containing both upper and lower case variants.
     */
...
public:
    /**
     * @brief Allocates and initializes a new NFA state managed by this builder.
     * @return Pointer to the newly created NFAState object.
     */
    ...
    /**
     * @brief Constructs a primitive NFA fragment matching specified character ranges.
     * @param ranges Set of valid byte range transitions.
     * @param apply_case If true, expands character ranges for case insensitivity.
     * @return NFAFragment matching the range.
     */
    ...
    /**
     * @brief Concatenates two NFA fragments sequentially (\c AB ).
     * @param first The prefix NFA fragment.
     * @param second The suffix NFA fragment.
    ...
    /**
     * @brief Constructs an alternation NFA fragment (\c A|B ).
     * @param first Left choice NFA fragment.
     * @param second Right choice NFA fragment.
     * @return Combined NFA fragment matching either branch.
     */
    ...
    /**
     * @brief Constructs a Kleene star repetition fragment (\c A* ).
     * @param frag Base NFA fragment to repeat 0 or more times.
     * @return Fragment representing zero-or-more repetition.
     */
    ...
    /**
     * @brief Constructs a Kleene plus repetition fragment (\c A+ ).
     * @param frag Base NFA fragment to repeat 1 or more times.
     * @return Fragment representing one-or-more repetition.
     */
    ...
    /**
     * @brief Constructs an optional fragment (\c A? ).
     * @param frag Base NFA fragment to match zero or one time.
     * @return Fragment representing zero-or-one occurrence.
     */
    ...
    /**
     * @brief Compiles a full regular expression string into an NFA fragment graph.
     * @param pattern The regular expression source pattern.
     * @param case_insensitive Enables case-insensitive matching across the expression.
     * @return Entire constructed NFAFragment.
     * @throws std::runtime_error On syntax error or trailing unexpected input.
     */
...
private:
    /**
     * @brief Parses top-level regex alternation expressions (\c term1|term2|... ).
     */
    ...
    /**
     * @brief Parses concatenation of adjacent regex atomic terms (\c atom1atom2... ).
     */
    ...
    /**
     * @brief Parses quantified regex atoms (\c atom* , \c atom+ , \c atom? , \c atom{m,n} ).
     */
    NFAFragment parseQuantifiedAtom(const std::string& pat, size_t& pos) {
        ...
        if (c == '{') {
            ...
            if (close == std::string::npos) throw std::runtime_error("Unmatched '{'");
    ...
    /**
     * @brief Parses primary regex units (character literals, character classes, shorthand macros, grouping).
     */
    ...
    /**
     * @brief Parses explicit bracketed character classes (\c [a-z0-9_] or \c [^A-Z] ).
     */
...
/**
 * @struct DFAState
 * @brief Represents a single state in a Deterministic Finite Automaton (DFA).
 */
struct DFAState {
    int id;               /**< DFA state ID index (0 represents dead state). */
    bool is_accept = false;/**< Accepting state flag. */
    int token_id = 0;     /**< Winning rule token identifier upon acceptance. */
    bool anchor_bol = false; /**< Inherited start-of-line anchor constraint. */
    bool anchor_eol = false; /**< Inherited end-of-line anchor constraint. */
    std::map<unsigned char, int> transitions; /**< Map of input byte values to target DFA state IDs. */
};

/**
 * @class DFAConverter
 * @brief Converts Non-deterministic Finite Automata (NFA) to Deterministic Finite Automata (DFA) using Powerset Construction.
 */
class DFAConverter {
private:
    std::vector<std::unique_ptr<DFAState>> dfa_states_; /**< Memory pool of converted DFA states. */

    /**
     * @brief Computes the epsilon-closure (\f$\epsilon\f$-closure) of a given set of NFA states.
     * @param states Input set of NFA states.
     * @return Complete set of reachable states through zero or more epsilon transitions.
     */
...
public:
    /**
     * @brief Converts an NFA graph into a equivalent deterministic state graph.
     * @param master_start Root start state of the master NFA combining all lexing rules.
     * @return Vector of raw pointers to constructed DFA state objects.
     */
    std::vector<DFAState*> convertNFA(NFAState* master_start) {
        dfa_states_.clear();

        // State 0 reserved as the dead state
 ...
 * @brief Holds character equivalence mapping data for transition table compression.
 */
struct EquivalenceClassResult {
    int byte_to_class[256];                            /**< Lookup mapping input byte values to equivalence class IDs. */
    int num_classes;                                   /**< Total number of distinct input character classes. */
    std::vector<std::vector<int>> compressed_transitions; /**< Transition table indexed by [state][class_id]. */
};

/**
 * @brief Identifies identical column behaviors across all DFA states to compute byte equivalence classes.
 * @param dfa Vector of un-minimized DFA state pointers.
 * @return EquivalenceClassResult structure containing mapped character classes and compressed table.
 */
...
/**
 * @brief Minimizes a DFA graph using Hopcroft's partition-refinement algorithm.
 * @param states Input un-minimized DFA states.
 * @param eq Computed byte equivalence classes for transition lookup.
 * @return Vector of unique pointers owning the minimized DFA states.
 */
...
/**
 * @class RegexCompiler
 * @brief High-level facade for defining lexing rules and compiling them into a standalone C++ Tokenizer class.
 */
class RegexCompiler {
private:
    /**
     * @struct Rule
     * @brief Internal tracking structure for registered regex rules.
     */
    struct Rule {
        std::string pattern;      /**< Regex pattern string. */
        int id;                   /**< Target token ID. */
        bool case_insensitive;    /**< Case insensitivity flag. */
        bool anchor_bol = false;  /**< Anchor at start-of-line flag. */
        bool anchor_eol = false;  /**< Anchor at end-of-line flag. */
    };
    std::vector<Rule> rules;      /**< Collection of registered lexing rules. */

public:
    /**
     * @struct RuleSpec
     * @brief Specification descriptor for bulk adding lexical rules.
     */
    struct RuleSpec {
        std::string pattern;      /**< Regex pattern string. */
        int token_id;             /**< Unique ID assigned to matched tokens. */
        bool case_insensitive = false; /**< Toggles case folding. */
        bool anchor_bol = false;  /**< Requires start-of-line match. */
        bool anchor_eol = false;  /**< Requires end-of-line match. */
    };

    /**
     * @brief Adds a single lexical rule to the compiler pipeline.
     * @param pattern The regular expression pattern.
     * @param token_id Integer ID returned when this token pattern is matched.
     * @param case_insensitive Optional flag enabling case-insensitive matching.
     * @param anchor_bol Requires rule match at the start of a line.
     * @param anchor_eol Requires rule match at the end of a line.
     */
    ...
    /**
     * @brief Adds multiple rules to the compiler in bulk.
     * @param rule_list Initializer list of RuleSpec descriptors.
     */
    ...
    /**
     * @brief Compiles all rules and generates C++ source header code for a standalone Tokenizer class.
     * @param class_name Desired name for the emitted C++ class (defaults to "Tokenizer").
     * @return Complete C++ class header code as a string stream.
     */
    std::string generateCppClass(const std::string& class_name = "Tokenizer") {
        ...
        ss << "public:\n";
        ss << "    /**\n     * @struct Token\n     * @brief Lexical token produced by the generated tokenizer.\n     */\n";
        ss << "    struct Token {\n";
        ss << "        int id;           /**< Matched token rule identifier. */\n";
        ss << "        std::string text; /**< Lexeme text content. */\n";
        ss << "        size_t position;  /**< Zero-based byte offset in source string. */\n";
        ss << "        size_t line;      /**< One-based line number. */\n";
        ss << "        size_t col;       /**< One-based column offset. */\n";
        ss << "        int file;         /**< Source file descriptor/identifier. */\n\n";

        ss << "        [[nodiscard]] bool is(int k) const { return id == k; }\n";
        ss << "        [[nodiscard]] bool isNot(int k) const { return id != k; }\n";
        ss << "        [[nodiscard]] bool is(std::string_view s) const { return s == text; }\n";
        ss << "        [[nodiscard]] bool isNot(std::string_view s) const { return s != text; }\n";
        ss << "    };\n\n";

        ss << "    /**\n";
        ss << "     * @brief Tokenizes an input string into a stream of tokens.\n";
        ss << "     * @param input Source text to analyze.\n";
        ss << "     * @param file ID representing source file origin.\n";
        ss << "     * @return Vector of matched Token objects.\n";
        ss << "     */\n";
        ...
        ss << "                tokens.push_back({-2, std::string(1, bad_char), start_pos, current_line, current_column, file});\n";
        ss << "                pos = start_pos + 1;\n";
        ss << "                \n";
        ss << "                if (bad_char == '\\n') { current_line++; current_column = 1; }\n";
        ss << "                else { current_column++; }\n";
